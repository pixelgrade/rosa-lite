    ...
    class ReduxFramework {
        ...
        public function __construct( $sections = array(), $args = array(), $extra_tabs = array() ) {
            ...
            if ( !empty( $this->args['opt_name'] ) ) {
                ...
                if ( isset( $_GET['page'] ) && $_GET['page'] == $this->args['page_slug'] || isset( $GLOBALS['wp_customize']) ) {
...
//                $comparison = $field['required'][1];
            ...
            foreach( $this->sections as $section ) {
                if( isset( $section['fields'] ) ) {
                    foreach( $section['fields'] as $field ) {
						...
						// @TODO Make a config which will exclude some fields from theme options and display them only in customizer
						if ( $field['type'] == 'customizer_info' ) {
						    continue;
						}
        ...
        public function section_menu($k, $section, $suffix = "") {
          ...
          if (isset($section['type']) && $section['type'] == "divide") {
	        ...
	        $this_href = 'javascript:void(0);';

            // DOVY! REPLACE $k with $section['ID'] when used properly.
            $string .= '<li id="' . $k.$suffix . '_section_group_li" class="redux-group-tab-link-li">';
            $string .= '<a href="' .$this_href. '" id="' . $k.$suffix . '_section_group_li_a" class="redux-group-tab-link-a" data-rel="' . $k.$suffix . '">' . $icon . '<span class="group_title">' . $section['title'] . '</span></a>';
