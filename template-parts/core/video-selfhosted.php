...
<video <?php echo ! empty( $video_poster ) ? 'poster="' . $video_poster . '"' : ''; ?> width="100%" height="auto"
	...
	<object width="100%" height="auto" type="application/x-shockwave-flash"
		data="<?php echo rosa::coreresourceuri( 'general-media/flashmediaelement.swf' ) ?>">
		<param name="movie" value="<?php echo rosa::coreresourceuri( 'general-media/flashmediaelement.swf' ) ?>"/>
		<?php if ( $video_m4v != '' ) : ?>
			<param name="flashvars" value="controls=true&file=<?php echo $video_m4v; ?>"/>
		<?php endif; ?>
		<!-- Image as a last resort -->
		<img
			src="<?php echo ! empty( $video_poster ) ? $video_poster : rosa::coreresourceuri( 'general-media/mediaelement/no-video-image.jpg' ); ?>"
