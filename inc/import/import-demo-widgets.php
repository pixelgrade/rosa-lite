...
if ( $wpGrade_importerError !== false ) {
	...
	if ( ! is_file( $import_filepath . '.php' ) ) {
		$response['id'] = new WP_Error( 'import_widgets_menus_nofile', 'The PHP file containing the widgets data could not be found or could not be read in <pre>' . rosa::themefilepath( 'inc/import/demo-data' ) . '</pre><br/> You might want to try to set the file permission to 777.' );
	} else {
		ob_start();
