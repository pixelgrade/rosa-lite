 ...
 * So we add a transient which will be handled by the `rosa_admin_redirect_to_pixcare_install_once` action
 */
function rosa_force_redirect_to_pixcare_install_once() {
	...
	if ( empty( $plugin_version ) ) {
		set_transient( '_rosa_activation_redirect', 1 );
	}
}
add_action( 'after_switch_theme', 'rosa_force_redirect_to_pixcare_install_once' );

/**
 *  * Redirect the admin to the pixcare install page once only if the plugin is missing
 */
function rosa_admin_redirect_to_pixcare_install_once() {
	if ( ! get_transient( '_rosa_activation_redirect' ) ) {
		return;
	}
	delete_transient( '_rosa_activation_redirect' );
...
add_action( 'admin_init', 'rosa_admin_redirect_to_pixcare_install_once' );

function rosa_add_pixcare_install_page() {
	add_submenu_page( null, '', '', 'manage_options', 'pixelgrade_care-install', null );
}
add_action( 'admin_menu', 'rosa_add_pixcare_install_page' );

function rosa_pixcare_install_page() {
...
add_action( 'admin_init', 'rosa_pixcare_install_page' );
